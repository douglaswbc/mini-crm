...
async function renderWhatsApp(body, tenant) {
  ...
  const box = el('div', { className: 'panel', style: 'padding:16px' }, [
    el('div', { style: 'font-size:13px;line-height:2' }, [
      ...
      el('div', null, [
        'Webhook da instância (apontado automaticamente ao conectar): ',
        el('code', { textContent: status.webhook_url || '(Evolution não configurada no servidor)' }),
      ]),
      el('div', { style: 'margin-top:8px;color:var(--muted)', textContent: 'A instância é criada com as credenciais do servidor (EVOLUTION_BASE_URL + GLOBAL_API_KEY do .env). Você não precisa digitar ID ou token.' }),
    ]),
  ]);

  const form = el('div', { className: 'panel', style: 'padding:16px;margin-top:12px' }, [
    el('b', { textContent: 'Configuração' }),
...
async function saveWaConfig(tenant) {
  try {
    ...
    waSetStatus('Configuração salva. Clique em "Criar instância" para provisionar na Evolution.');
...
async function deleteWaInstance(tenant) {
  if (!confirm('Excluir a instância (na Evolution) e remover a configuração deste cliente?')) return;
  try {
    const payload = user.role === 'admin' ? { tenant_id: tenant.id } : {};
