...
// As credenciais ficam no .env do servidor (o painel não pede ID/token).
...
// ---------------------------------------------------------------------------
// WhatsApp — instância (Evolution API) + webhook
// O CRM usa as credenciais da Evolution do .env do servidor para criar,
// conectar (apontar o webhook), mostrar o QR, consultar status e excluir.
...
app.get('/api/whatsapp/status', requireSession, requireClientScope, async (req, res) => {
  ...
  let live = null;
  if (inst && waInstanceId(inst) && inst.instance_token && evolutionConfigured()) {
    try {
...
app.post('/api/whatsapp/connect', requireSession, requireClientScope, async (req, res) => {
  ...
  if (!tenantId) return res.status(400).json({ error: 'Tenant não informado ou sem acesso.' });
  const inst = await db.getWhatsappInstanceByTenant(tenantId);
  if (!inst || !inst.instance_token || !waInstanceId(inst)) {
